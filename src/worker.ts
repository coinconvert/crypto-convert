...
class PricesWorker {
	...
	options: Options = {
		cryptoInterval: isBrowser ? 15 * 1e3 : (5 * 1e3), //Every 5 seconds on server/Every 15 seconds on browser
	 ...
	 * Options
	 */
	setOptions(o?: Options | ((currentOptions: Options) => Options)) {
	...
	async runServer() {
		...
		this.crypto_worker = setInterval(
			this.updateCrypto.bind(this), this.options.cryptoInterval
		);
